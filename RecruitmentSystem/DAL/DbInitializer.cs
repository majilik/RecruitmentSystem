...
namespace RecruitmentSystem.DAL
{
    public class DbInitializer : DropCreateDatabaseIfModelChanges<RecruitmentContext>
    {
        protected override void Seed(RecruitmentContext context)
        {
            ...
            {
                new Availability {Person=context.Persons.Where(p => p.Id == 2).Single(), FromDate=DateTime.Parse("2014-02-23"), ToDate=DateTime.Parse("2014-05-25")},
                new Availability {Person=context.Persons.Where(p => p.Id == 2).Single(), FromDate=DateTime.Parse("2014-07-10"), ToDate=DateTime.Parse("2014-08-10")}
            };
